---
title: "Connect Apps"
description: "Connect 40+ apps to Pane so the assistant can work with your email, calendar, projects, and more"
---

Connect your favorite apps to Pane and let the assistant work across all of them. Read emails, check your calendar, create tasks, post messages, manage files, and more, all through natural conversation.
...
Pane uses the [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) to connect your apps. You do not need to install anything or manage API keys. Just sign in once and the assistant handles the rest.
...
Pane connects to over 40 apps across every category you need.
...
A browser window will open to sign in. Once authenticated, add `http://localhost:8000/sse` as a custom MCP in Pane.

<Note>
Keep the terminal running while you use Pane. The local server handles authentication and proxies requests to the remote MCP server.
</Note>
...
<Columns cols={2}>
  <Card title="Secure OAuth" icon="shield-check">
    All apps use OAuth sign-in. Pane never sees or stores your passwords.
  </Card>
