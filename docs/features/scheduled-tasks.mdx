...
description: "Run the Pane agent automatically on a schedule"
---

Scheduled Tasks let you run the Pane agent automatically, whether it is daily, every few hours, or every few minutes. Write a prompt once, set a schedule, and let the agent handle it on autopilot.
...
<Steps>
  <Step title="Task triggers on schedule">
    Pane uses your browser's built-in alarm system to trigger tasks at the right time. If your laptop was closed at the scheduled time, the task runs as soon as you open Pane again.
  </Step>
...
<Note>
Pane needs to be open for scheduled tasks to run. Tasks have a 10-minute timeout. If a task takes longer than that, it will be marked as failed and you can retry it.
</Note>
