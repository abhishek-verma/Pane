...
## Final live verification (2026-09-09)

Production Codex routing and budget disclosure are integrated. After the public PR's source hashes were verified, automatic approval review accepted the direct fixed-sample live requests. The configured Codex account passed typed translation and the bounded inspect/execute/complete protocol with a synthetic page host. API, Claude and Codex each pass 52 real-browser fixture checks. The earlier Codex backend/schema failure is resolved.

The configured Claude account's live smoke is denied by organization subscription policy, despite being authenticated. The adapter reports this as nonretryable PROVIDER_ACCESS_DENIED with a safe actionable explanation; no alternative credentials/provider are used. Success is verified through installed-CLI/local-API and full browser fixtures; a live success cannot be asserted for this restricted account. See ../RELEASE-REVIEW.md for current disposition.
