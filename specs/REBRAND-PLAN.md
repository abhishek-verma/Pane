...
- [x] **S3.2 — Community + agent URLs:** Pane Discord is `https://discord.gg/652JHSyS4` in `productUrls.ts` (and docs/UI). Slack invite still `TODO(pane-infra)` / hidden. `apps/app/modules/chat/chat-types.ts` agent-suggestion URLs → Pane URLs (separate).
