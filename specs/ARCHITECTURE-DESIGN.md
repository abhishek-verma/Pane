...
1. **(Fixed)** **`chrome://browseros/mcp` is broken.** The Chromium patch maps it to `app.html#/mcp`, but `entrypoints/app/App.tsx` has **no `/mcp` route** → catch-all redirects to `/home`. The real page is `#/settings/mcp`. **Fix:** add a `/mcp` route (alias to the MCP settings view) or repoint the patch. This is the wedge's front door.
2. **(Fixed)** **`claude mcp add` setup UI is gated behind `VITE_KLAVIS_INTTEGRATIONS`.** The dev wedge's one-line setup shouldn't depend on the Connect Apps flag. **Fix:** move `QuickSetupSection` out from under `IntegrationsSection`'s Klavis gate; keep URL copy + `claude mcp add` snippets always visible.
3. **(Fixed)** **Sidepanel sessions are in-memory only.** A server restart loses chat history. **Fix:** persist sessions to SQLite (extend `lib/db/`) — required before memory/session-archive can work.
4. **(Fixed)** **`produced_files` is half-wired** (SQL-only, no Drizzle schema module). **Fix:** add the schema module — and use the same pattern for new tables.
