...
**Execution plan:** [Pane Layers implementation plan](./PANE-LAYERS-IMPLEMENTATION-PLAN.md) is the authoritative dependency-ordered work breakdown, with LP-00–LP-13 file-level tasks, tests and full-scope release gates. The L0–L7 table below remains a capability overview.

**Required follow-up:** [Capability honesty, skills, and provider parity](./21-pane-layers-capabilities-and-skills.md) defines request assessment before promises, a thin prompt plus focused skills, and native/Claude Code/Codex parity as a release gate. These requirements apply to the implementation sequence below.
...
An on-demand `page-task` can inspect the current document, apply managed mutations or explicitly granted generated JavaScript, verify the visible result, and return a typed completion receipt. Its button/intent persists while generated effects default to the current document session. The runtime records mutation ownership and actual verification references; a missing final response does not imply that nothing changed. Arbitrary scripts retain the advanced mode's cleanup limitations. Saving generated code for future automatic execution is a separate versioned authoring operation, not an implicit result of clicking the button.
...
| **L2 — Agent authoring and preview** | Shared native/MCP tool registry, canonical Layer skills with thin prompt routing, capability assessment, DOM anchor capture/picker, draft validation, independent `layer_test`/runtime inspection, verification receipts, explicit-persistence handling, version replacement, Edit with Pane, bounded repair and rollback. | Native, Claude Code and Codex discover skills/tools without Layer-specific setup; unsupported requests are reported honestly; activation references evidence for that exact version; failed edits leave the old version intact. |
| **L3 — Agentic buttons** | Named transform/page-task actions, bounded runner with native/Claude Code/Codex adapters, document binding, cancellation/dedupe, cost/provider UI, trusted approvals, translation renderer/cache and invocation-scoped result submission. | All three supported provider paths execute their advertised actions without separate user setup; original survives failures; forged requests and stale results are rejected. |
