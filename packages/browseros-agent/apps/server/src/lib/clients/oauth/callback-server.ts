  ...
  async ensureRunning(): Promise<void> {
    ...
    throw new Error(
      `OAuth callback port ${OAUTH_CALLBACK_PORT} is in use by another process. ` +
        'Close other Pane instances or CLI tools and try again.',
    )
  }
...
function successPage(): string {
  return `<!DOCTYPE html>
<html><head><title>Pane - Authentication Successful</title>
...
<body><div class="card"><h1>Authentication Successful</h1><p>You can close this tab and return to Pane.</p></div></body></html>`
}
...
function errorPage(message: string): string {
  const escaped = message
    ...
    .replace(/>/g, '&gt;')
  return `<!DOCTYPE html>
<html><head><title>Pane - Authentication Failed</title>
<style>body{font-family:system-ui;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#f8f9fa}
.card{text-align:center;padding:2rem;background:white;border-radius:12px;box-shadow:0 2px 8px rgba(0,0,0,0.1)}
