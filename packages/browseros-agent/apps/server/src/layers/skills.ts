const LAYER_STATE_GUIDANCE = `For existing Layers, call layer_list yourself first. It reports enabled (the effective browser toggle), savedEnabled (the server record), disabledLocally, status (enabled/disabled/paused/setup-incomplete), global/site pauses, script execution status, and the revision. You can toggle Layers with layer_enable and layer_disable; do not ask the user to check or toggle them manually. Use layer_set_paused to resume a requested site or global pause. Enabling does not override pauses. An incomplete draft must pass preview and verification before it can be enabled. An explicit layer_preview resumes a disabled unfinished draft for testing without installing it. If preview reports a local disable on an already verified Layer, use layer_enable before retrying; if scripts report reload-required, reload the originating page and refresh layer_tabs before retrying. Never keep retrying an authorization error without inspecting its cause. If a button action starts and fails, call layer_activity and read failureCode/failureReason before changing the Layer. Provider/model/tool-boundary failures need provider or adapter repair, not repeated previews, duplicate Layers or toggle requests. Claude model aliases such as sonnet are valid saved selections; do not ask the user to replace an alias with a guessed model ID. If the browser connection is unavailable, report that state could not be observed; do not infer that the Layer is disabled. An enabled toggle is not proof that effects are mounted: inspect script status and the live page. After testing disable behavior, restore the user's requested state with a fresh revision, reload if required, and confirm it with layer_list. Newly created or repaired Layers should finish enabled unless the user requested otherwise.`
