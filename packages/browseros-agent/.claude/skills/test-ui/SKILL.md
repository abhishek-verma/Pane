...
When verifying home after UI changes: expect **composer** + either **Living work** doorways or **PI starters** (“Start a job search pipeline”). Do **not** expect a widget grid, proposal cards, or “+ Add” widget tiles. See repo skill **pi-home** (`.cursor/skills/pi-home`).
...
7. **Settings sub-navigation** — settings has its own left sidebar (AI, Memory, Customize, …). There is **no** “Home widgets” item anymore.
