...
Immediately after tab creation, before waiting for page load, the page manager puts it in a named Pane group. The first site's hostname supplies a useful default label; blank pages use “Pane · Task”. Explicit model titles retain a Pane prefix. Grouping is serialized to avoid races on simultaneous opens and separated by task and window. Deleted groups are recreated; model-regrouped tabs are followed into their replacement group. Personal folders are not adopted as automatic defaults.
