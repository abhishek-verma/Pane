#!/usr/bin/env python3
"""DMG creation and packaging module for Pane"""
...
class MacOSPackageModule(CommandModule):
    ...
    def _create_dmg(self, app_path: Path, dmg_path: Path, pkg_dmg_path: Path) -> None:
        if not create_dmg(app_path, dmg_path, "Pane", pkg_dmg_path):
...
def package_universal(contexts: List[Context]) -> bool:
    ...
    if create_dmg(universal_app_path, dmg_path, "Pane", pkg_dmg_path):
