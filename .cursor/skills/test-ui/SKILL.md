---
name: test-ui
description: Test the Pane app extension UI by starting the dev environment and visually verifying changes via CDP. Covers the new tab page (left sidebar — Home, Scheduled Tasks, Settings, etc.) and the right side panel (chat interface). Use after making UI changes to apps/app/.
---

# Test App UI

Visually test the Pane app extension UI — both the new tab page (left sidebar) and the right side panel (chat) — by starting the dev environment and inspecting via CDP.
...
- `[page] chrome-extension://biedncddmddkpapdplhcnkhhplnfgbif/app.html#/...` — **New tab page (left sidebar)**
...
The side panel starts **disabled** in a fresh profile. Open it using Pane-specific APIs:
